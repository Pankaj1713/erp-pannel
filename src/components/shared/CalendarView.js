...
import { colorBookingStatus, getFirstDateOfMonth } from "utils/helpers";
...
const CalendarView = (props) => {
  const { wrapperClass, startDate, ...rest } = props;
  ...
  useEffect(() => {
    if (startDate) {
      ...
      handleChangeMonth(firstDateOfMonth);
    }
  }, [startDate]);
  return (
    <div className={classNames("calendar", wrapperClass)}>
      <FullCalendar
        ...
        eventContent={(arg) => {
          ...
          const colorBookingStatus = (status) => {
            if (status === 2) {
              return {
                bg: "bg-green-50 dark:bg-green-500/10",
                text: "text-green-500 dark:text-green-100",
          ...
          const defaultColor = colorBookingStatus(extendedProps?.status);
            ...
            <div
              className={classNames(
                ...
                extendedProps?.status
                  ? colorBookingStatus(extendedProps?.status)?.bg
                  : defaultColor?.bg,
                extendedProps?.status
                  ? colorBookingStatus(extendedProps?.status)?.text
                  : defaultColor?.text,
                isEnd &&
                  !isStart &&
            ...
            >
              {!isEnd && !isStart && (
                <Badge
                  className={classNames(
                    "mr-1 rtl:ml-1",
                    extendedProps?.bookingId?.status
                      ? colorBookingStatus(extendedProps?.bookingId?.status)
                          ?.dot
                      : defaultColor?.dot
                  )}
                />
              ...
              {!(isEnd && !isStart)}
              <p className="font-semibold ml-1 rtl:mr-1">
                {arg.event.title}-{extendedProps?.bookingId?.bookingNo}{" "}
                <br></br>({extendedProps?.orderNumber})
              </p>
            </div>
          );
        }}
