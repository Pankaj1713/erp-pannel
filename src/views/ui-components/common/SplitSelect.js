...
export default function SplitSelect({ BarChartBtn = false, options=[] }) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [selectedStore, setSelectedStore] = useState(options.length > 0 ? options[0].name : "");
  const anchorRef = useRef(null);

  const filteredStores = options.filter((store) =>
